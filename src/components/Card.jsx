...
export default function CardComp({ title, link, cardIndex, bucketIndex, setModal, rerenderOnce, cardId }) {
  ...
  return (
    <Card style={{ marginLeft: "10px", marginTop: '1rem', position: "relative", textOverflow: 'ellipsis', overflow: 'hidden' }} sx={{ maxWidth: 300, height: 'min-content', opacity: isDragging ? 0.5 : 1 }} ref={drag}>
      <CardContent style={{textOverflow: 'ellipsis', overflow: 'hidden'}}>
        <Typography gutterBottom variant="h5" component="div">
          {title}
        </Typography>
        <MenuList style={{color: '#6d8c9f'}} moveCardTo={moveCardTo} moveCardFrom={moveCardFrom} />
      ...
      <CardActions style={{
        ...
        <Button size="small" style={{color: '#6d8c9f'}} onClick={handleClick}>Watch</Button>
        <Button size="small" style={{color: '#6d8c9f'}} onClick={handleEdit} > <EditIcon /></Button>
        <Button style={{color: '#6d8c9f'}} onClick={() => {
